...
p = ggplot(plotVolc, aes(x = log2FoldChange, y = -log10(padj), label = geneLabel))+
  ...
  theme_few()+
  geom_text_repel(max.overlaps = Inf, min.segment.length = 0, force = 20)+
  labs(title = 'MDAMB231 DE Genes') + 
  theme(legend.position = "none")

ggsave('../../figures/final/231VolcanoPlot.png', p, dpi = 500)
...
xticks = round(seq(min(plotVolc$log2FoldChange), max(plotVolc$log2FoldChange)))
ggplot(plotVolc, aes(x = log2FoldChange, y = -log10(padj), label = geneLabel))+
  geom_point(aes(color = factor(color)))+
  scale_color_manual(values = cols)+
  geom_vline(xintercept = c(-l2fc,l2fc), linewidth=0.1, linetype='dashed') +
  geom_hline(yintercept = c(-log10(padj)), linewidth=0.1, linetype='dashed') +
  ...
  theme_few()+
  geom_text_repel(max.overlaps = Inf, min.segment.length = 0, force = 20)+
  labs(title = 'MDAMB436 DE Genes') + 
  theme(legend.position = "none")

ggsave('../../figures/final/436VolcanoPlot.png', p, dpi = 500)
